...
export default function Department() {
    ...
    const columns: ColumnsType<IDepartment> = [
        ...
        },
        {
            ...
            dataIndex: 'action',
            align: 'center',
            render: (_: any, record: IDepartment) => !isArchive ? <Action
                title='department'
                name={record.name}
            ...
            /> : <Popconfirm
                ...
                onConfirm={() => {
                    GET(HRSETTINGS.DEPARTMENT.RESTORE + record?.id)
    ...
    function fetchData(args?: IArguments) {
        setLoading(true)
        let url = args?.isArchive ? (HRSETTINGS.DEPARTMENT.GET + '/archives') : HRSETTINGS.DEPARTMENT.GET;
        GET<DepartmentRes>(url, args?.signal!, { page: args?.page!, search: args?.search!, limit: args?.pageSize! })
            .then((res) => {
                setData(res?.data ?? [])
                setTableParams({
                    ...tableParams,
    ...
    return (
        <Card title={`Departments ${isArchive ? '- Archives' : ''}`}>
            <TabHeader
                handleSearch={setSearch}
                handleCreate={!isArchive ? () => setIsModalOpen(true) : undefined}
                handleModalArchive={!isArchive ? () => setIsArchive(true) : undefined}
            >
                {isArchive ? <Button onClick={() => setIsArchive(false)}>Back to departments</Button> : null}
            </TabHeader>
            {!isArchive ? (
                <>
                    <Table
