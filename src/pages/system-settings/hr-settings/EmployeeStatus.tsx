...
export default function EmployeeStatus() {
    ...
    const columns: ColumnsType<IEmployeeStatus> = [
        ...
        },
        {
            ...
            dataIndex: 'action',
            align: 'center',
            render: (_: any, record: IEmployeeStatus) => !isArchive ? <Action
                title='employee status'
                name={record.name}
                onConfirm={() => handleDelete(record.id)}
                onClick={() => handleEdit(record)}
            /> : <Popconfirm
                title={`Restore employee status`}
                description={`Are you sure you want to restore ${record?.name}?`}
                onConfirm={() => {
                    GET(HRSETTINGS.EMPLOYEESTATUS.RESTORE + record?.id)
    ...
    function fetchData(args?: IArguments) {
        setLoading(true)
        let url = args?.isArchive ? (HRSETTINGS.EMPLOYEESTATUS.GET + '/archives') : HRSETTINGS.EMPLOYEESTATUS.GET;
        GET<EmployeeStatusRes>(url, args?.signal, { page: args?.page!, search: args?.search!, limit: args?.pageSize! })
            .then((res) => {
                setData(res?.data ?? [])
                setTableParams({
                    ...tableParams,
    ...
    return (
        <Card title={`Employee Status ${isArchive ? '- Archives' : ''}`}>
            <TabHeader
                handleSearch={setSearch}
                handleCreate={!isArchive ? () => setIsModalOpen(true) : undefined}
                handleModalArchive={!isArchive ? () => setIsArchive(true) : undefined}
            >
                {isArchive ? <Button onClick={() => setIsArchive(false)}>Back to employee status</Button> : null}
            </TabHeader>
            {!isArchive ? (
                <>
                    <Table
